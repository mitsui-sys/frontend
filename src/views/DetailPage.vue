<template>
  <v-card id="detail" width="500" justify="center">
    <v-card-title>
    ...
    <v-divider></v-divider>
    <v-card-text>
      <v-container style="max-height: 700px" class="overflow-y-auto" outlined>
        <v-row v-for="(item, index) in items" :key="index" no-gutters>
          <v-subheader width="200px">{{ item.text }}</v-subheader>
          <v-text-field
            v-model="item.value"
            placeholder="値を入力"
            outlined
